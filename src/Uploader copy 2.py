...
def main():
    ...
    try:
        ...
        csv_file_path = 'E:\\New Scrapper fragrancedirect\\SAMPLEEEE.csv'  # Replace with the actual path to your CSV file
        csv_file_pathh = 'E:\\New Scrapper fragrancedirect\\SAMPLEEEE copy.csv'  # Replace with the actual path to your CSV file
        titles_list = []
        # Open the CSV file
        ...
        with open(csv_file_path, "r", encoding="latin1") as file:
            ...
            for row in reader:
                ...
                title = decode_special_characters(row[0])
                try:
                    title = decode_special_characters(row[0])
                    title1 = re.sub(r'\s\d+ml', '', title)
                    # Append the value to the list
                except IndexError:
                    # Handle the case where the row might not have the expected number of columns
                    print(f"Skipping row {reader.line_num} due to IndexError")
                    
                    
                Desscription = decode_special_characters(row[1])
                Category = decode_special_characters(row[2])
                # Coollection = decode_special_characters(row[3])
                Taggs = decode_special_characters(row[4])
                # Taggs = row[4].split(',')
                pro_typpe = row[5]
                vendor = row[6]
                Prrice = row[7]
                weeight = row[8]
                skunumber = row[9]
                target_directory = decode_special_characters(row[10])
                # Coollection2 = decode_special_characters(row[11])
                ...
                #         action_chains.send_keys(Keys.ENTER).perform()
                #         try:
                #              with open(csv_file_pathh, 'r') as file:
                ...
                #                         Collectionnn.enter_name(Uploaderdata.Collection, Coollection)
                ...
                #                         time.sleep(1)
                #                         Collectionnn = HomePage(driver)
                #                         Collectionnn.enter_name(Uploaderdata.Collection, Coollection2)
